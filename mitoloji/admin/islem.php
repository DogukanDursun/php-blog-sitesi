...
//SPONSOR GÜNCELLE
    ...
    unlink("../../blogscripti\img/sponsorlar/".$eski_resim["sponsor_resim"]);
            ...
            if ($kaynak) {
                 if (!in_array($turu, $DosyaTuru) && !in_array($uzanti, $DosyaUzanti)) {
                    ...
                    if (move_uploaded_file($kaynak, $hedef)) {
                        ...
                        $query = $db->prepare("INSERT INTO yazilar SET yazi_foto=?, yazi_baslik=?, yazi_kategori_id=?,yazi_icerik=?, yazi_seolink=?");
        ...
        if ($kaynak) {
            if (!in_array($turu, $DosyaTuru) && !in_array($uzanti, $DosyaUzanti)) {
                ...
                $eski_resim = $sil->fetch(PDO::FETCH_ASSOC);
                    ...
                    if (move_uploaded_file($kaynak, $hedef)) {
                        $query = $db->prepare("UPDATE yazilar SET yazi_foto=?, yazi_baslik=?, yazi_kategori_id=?, yazi_icerik=?, yazi_seolink=? WHERE yazi_id=?");
            ...
            if (!$yazi_baslik || !$yazi_icerik) {
                ...
                $query = $db->prepare("UPDATE yazilar SET yazi_baslik=?, yazi_kategori_id=?, yazi_icerik=?, yazi_seolink=? WHERE yazi_id=?");
