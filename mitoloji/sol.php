...
<div id="column-1">
  ...
  $yazilar = $db->prepare("SELECT * FROM yazilar
    ...
    ORDER BY yazi_tarih DESC LIMIT $goster,$limit");
  ...
  foreach ($yazicek as $row) {
    ...
    $yorumlar->execute(array($row["yazi_seolink"],1));
    $yorumlar->fetchALL(PDO::FETCH_ASSOC);
    $yorum_say = $yorumlar->rowCount();
    ?>
    <div class="post-column">
       <a href="yazilar/<?php echo $row["yazi_seolink"]; ?>" title="<?php echo $row["yazi_baslik"]; ?>">
      ...
      <div class="post-column-bottom">
        <h1><a href="yazilar<?php echo $row["yazi_seolink"]; ?>" title="<?php echo $row["yazi_baslik"]; ?>"><?php echo $row["yazi_baslik"]; ?></a></h1>
        ...
        <div class="read-more">
            <h2><a href="yazilar/<?php echo $row['yazi_seolink']; ?>" class="read-more" title="Devamını Gor">DEVAMINI GÖR</a></h2>
        </div> -->
      </div>
    </div>
    <?php } ?>
